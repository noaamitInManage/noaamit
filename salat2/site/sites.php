...
include_once($_project_server_path . $_includes_path . 'class/siteFunctions.inc.class.php'); // 15/04/2011

$_Proccess_Main_DB_Table = "tb_sites";
...
$countries_sql = "
    SELECT `obj_id` AS `id`, `name` FROM `tb_countries_lang` WHERE `lang_id` = '{$_SESSION['lang_id']}' ORDER BY `name`
";
$countries_result = $Db->query($countries_sql);
$countriesArr = array();
while ($country_rowArr = $Db->get_stream($countries_result)) {
    $countriesArr[$country_rowArr['id']] = $country_rowArr['name'];
}
