...
$_Proccess_Main_DB_Table = "tb_" . $this_name . "s";
...
if ($act == 'new') {
    ...
    if ($_Proccess_Has_MultiLangs) {
        ...
        @unlink($_SERVER['DOCUMENT_ROOT'] . '/_static/' . $this_name . 's/' . get_item_dir($obj_id) . '/' . $languagesArr[$module_lang_id]['title'] . '/' . $this_name . '-' . $obj_id . '.inc.php');
    } else {
        $query = "DELETE FROM {$_Proccess_Main_DB_Table} WHERE id='{$obj_id}'";
        // delete static file !!!
        @unlink($_SERVER['DOCUMENT_ROOT'] . '/_static/' . $this_name . 's/' . get_item_dir($obj_id) . '/' . $this_name . '-' . $obj_id . '.inc.php');
    }
    $result = $Db->query($query);
    ...
    if (isset($_REQUEST['topcat_id']) && $_REQUEST['topcat_id'] > 0) {
...
function module_updateStaticFiles()
{
    ...
    /*
        if($_Proccess_Has_MultiLangs){
            $UpdateStatic = new testlvl3v2UpdateStaticFiles();
...
<head>
    ...
    <script type="text/javascript" src="/resource/uploadify/swfobject.js"></script>
                ...
                <tr>
                    <td class="dottTblS" colspan="3">
                        ...
                        <div id="loader" style="display:none;"><img
                                    src="/salat2/images/ajax-loader.gif"/> <?= $_LANG['LOADER_TXT']; ?></div>
                    </td>
                </tr>
            </table>
