...
$_Proccess_Main_DB_Table = "tb_translate";
...
//die('<hr /><pre>' . print_r($_REQUEST, true) . '</pre><hr />');
	...
	if($_Proccess_Has_MultiLangs){
		...
		if ($result->num_rows($multiLangRes) == 0){
...
if (isset($_REQUEST['dosearch']) && isset($_REQUEST['q'])){
	if ($_REQUEST['q'] != ''){
		...
		$searchTerm = "WHERE `key_code` LIKE '%{$searchTerm}%'"; ;
	}
}
...
// fetch all items 
$items = array();
$itemsSql = "SELECT * FROM `tb_translate` {$searchTerm} ORDER BY `id` DESC";
$itemsRes = $Db->query($itemsSql);
while($item = $Db->get_stream($itemsRes)){
	if (!is_array($items[$item['key_code']])){ $items[$item['key_code']] = array(); }
	$items[$item['key_code']][$item['lang_id']] = $item;
	ksort($items[$item['key_code']]);
}
...
include_once($_SERVER['DOCUMENT_ROOT'].'/salat2/_inc/module_info.inc.php');
?>
...
<head>
	...
	<script type="text/javascript" src="../_public/translations.js"></script>
	...
	<style type="text/css">
		...
		.ltr-input { direction:ltr !important; }
		.rtl-input { direction:rtl !important; }
...
<div class="maindiv">
	...
	// build table headers
	$tableHeader = '<tr class="dottTbl"><td>Key</td><td>GD</td>';
...
tableHeader;
	...
	<tr class="normTxt" id="newTranslation" >
		<td width="{$columnWidth}%"><textarea id="newTranslationKey" class="translated-text ltr-input"></textarea></td>
		<td>
			<input type="checkbox" name="show_in_gd" value="1">
		</td>
newRow;
		...
		<input type="button" id="saveNewTranslation" class="buttons" value="Add">
		<input type="button" id="cancelNewTranslation" class="buttons red" value="Cancel">
		</td>
	</tr>
newRow;
		echo $newTranslation;
		...
		foreach ($items as $key_code => $item) {
			...
			$first_items =reset($item);
			$show_in_gd = $first_items['show_in_gd'];
			$checked= ($show_in_gd) ? ' checked="checked" ' : '';
			$tableRow =  "<tr class='normTxt' data-keycode='{$key_code}'>";
			$tableRow .= "<td width='{$columnWidth}%'>{$key_code}</td>";
			$tableRow .= "<td width='5%'><input type=\"checkbox\" {$checked} class=\"gd_{$key_code}\" name=\"show_in_gd\" value=\"1\"></td>";
			$langItemArr=array();
			foreach($langs as $key=>$lang){
				...
				foreach($item as $keyItem=>$value){
					if($value['lang_id']==$lang['id']){
						...
						$text =  stripslashes(htmlspecialchars($value['text'],ENT_QUOTES));
						$directionClass = ( $lang['direction'] == 1? 'rtl-input' : 'ltr-input' );
						$tableRow .= <<<tableRow
                    <td width='{$columnWidth}%'><textarea data-langid="{$value['lang_id']}" class="translated-text {$directionClass}">{$text}</textarea></td>
tableRow;
						break;
					}
				}
			...
			foreach($langs as $key=>$lang){
				if(!in_array($lang['id'],$langItemArr)){
					...
					$tableRow .= <<<tableRow
                    <td width='{$columnWidth}%'><textarea data-langid="{$lang['id']}" class="translated-text {$directionClass}"></textarea></td>
tableRow;
				}
			}

			$tableRow .= '<td width="'.$columnWidth.'%"><input type="button" class="buttons saveTranslation" value="Save" >
	<input type="button" class="buttons red deleteTranslation" value="'.$_LANG['BTN_DEL'].'" >
	</td>';
			$tableRow .= '<tr>';
			echo $tableRow;
		}
