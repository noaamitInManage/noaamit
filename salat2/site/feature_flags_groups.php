...
$_Proccess_Main_DB_Table = "tb_feature_flags_groups";
...
//die('<hr /><pre>' . print_r($_REQUEST, true) . '</pre><hr />');
    ...
    if($_Proccess_Has_MultiLangs){
        ...
        @unlink($_SERVER['DOCUMENT_ROOT'].'/_static/feature_flags_groups/'.get_item_dir($obj_id).'/'.$languagesArr[$module_lang_id]['title'].'/feature_flag_group-'.$obj_id.'.inc.php');
    }else{
        $query="DELETE FROM {$_Proccess_Main_DB_Table} WHERE id='{$obj_id}'";
        // delete static file !!!
        @unlink($_SERVER['DOCUMENT_ROOT'].'/_static/feature_flags_groups/'.get_item_dir($obj_id).'/feature_flags_group-'.$obj_id.'.inc.php');
    }
    $result = $Db->query($query);
...
function module_updateStaticFiles() {
    ...
    $UpdateStatic = new featureFlagsGroupsUpdateStaticFiles();
