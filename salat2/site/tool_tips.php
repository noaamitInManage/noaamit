...
$_Proccess_Main_DB_Table = "tb_tool_tips";
...
//		$newOrder = mysqli_result($result,0,0);
	...
	if($_Proccess_Has_MultiLangs){
		...
		@unlink($_SERVER['DOCUMENT_ROOT'].'/_static/tool_tips/'.get_item_dir($obj_id).'/'.$languagesArr[$module_lang_id]['title'].'/tool_tip-'.$obj_id.'.inc.php');
	}else{
		$query="DELETE FROM {$_Proccess_Main_DB_Table} WHERE id='{$obj_id}'";
		// delete static file
		@unlink($_SERVER['DOCUMENT_ROOT'].'/_static/tool_tips/'.get_item_dir($obj_id).'/tool_tip-'.$obj_id.'.inc.php');
	}
	$result = $Db->query($query);
...
function module_updateStaticFiles() {
	...
	$UpdateStatic = new tool_tipsLangsUpdateStaticFiles();
...
$(function() {
   ...
   /*  auto media load */
	...
	$("#submitBtn, #stayBtn").on('click',function(){
		var keyValue = $("#eng_key").val();
		var pat = new RegExp("[^\u0000-\u0080]+");
		var isNonEnglish = pat.test(keyValue);
		if (isNonEnglish){
			$("#submitBtn, #stayBtn").show();
			$("#loader").hide();
			alert("Not english!");
			return false;
		}
	...
	<table width="100%" border="0" cellpadding="3" cellspacing="1" class="table-edit">
		...
		<tr>
			<td class="dottTblS" colspan="3">
				<input id="submitBtn" type="submit" name="send" value="<?php echo $submit;?>" class="buttons" onclick="document.getElementById('loader').style.display='';this.style.display='none';" />
				<input id="stayBtn" type="submit" name="stay" value="<?php echo $submitStay;?>" class="buttons" onclick="document.getElementById('loader').style.display='';this.style.display='none';" />
