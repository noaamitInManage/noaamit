...
$_Proccess_Main_DB_Table = "tb_feed__post_to_feed_options";
...
$post_to_feed_optionsArr = configManager::$post_to_feed_optionsArr;
...
//die('<hr /><pre>' . print_r($_REQUEST, true) . '</pre><hr />');
    ...
    if(isset($_FILES['picture']['tmp_name']) && ($_FILES['picture']['tmp_name']))
    {
        ...
        $image_name = md5($obj_file_id.time());
    ...
    {
        $image_url = $_REQUEST['picture_delete'];
        $target_file = $_SERVER['DOCUMENT_ROOT'].'/'.$image_url;
...
{
    $UpdateStatic = new postToFeedOptionsLangsUpdateStaticFiles();
