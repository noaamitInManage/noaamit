...
$_Proccess_Main_DB_Table = "tb_footer";
...
$footerParagraphs = array('-- Choose --');
$paragraphsSql = "SELECT * FROM `tb_footer_paragraph` AS  Main
							LEFT JOIN `tb_footer_paragraph_lang` AS Lang ON  (
								Main.`id`=Lang.`obj_id`
							)
						 WHERE  Lang.`lang_id`='{$module_lang_id}'";
$paragraphsRes = $Db->query($paragraphsSql);
while($paragraph = $Db->get_stream($paragraphsRes)){
	$footerParagraphs[] = $paragraph['paragraph_name'];
}
...
//die('<hr /><pre>' . print_r($_REQUEST, true) . '</pre><hr />');
	...
	if($_Proccess_Has_MultiLangs){
		...
		if (mysql_num_rows($multiLangRes) == 0){
...
function module_updateStaticFiles() {
	...
	if($_Proccess_Has_MultiLangs){
		$UpdateStatic = new footerLangsUpdateStaticFiles();
		$UpdateStatic->updateStatics();

		$UpdateStatic = new footerParagraphLangsUpdateStaticFiles();
		$UpdateStatic->updateAllStaticsFiles();
	}else{
		$UpdateStatic = new footerUpdateStaticFiles();
		$UpdateStatic->updateStatics();

		$UpdateStatic = new footerParagraphUpdateStaticFiles();
		$UpdateStatic->updateAllStaticsFiles();
	}
...
<?php  if($act=="show"){ ?>
	<table width="100%" border="0" cellpadding="3" cellspacing="1" style="empty-cells:show;" class="table-list">
		...
		<?php  for($count = mysql_num_rows($resultArr['result']),$i=0;$row = $Db->get_stream($resultArr['result']);$i++){
		$c=($i%2==0)?'even':'odd'; ?>
		<tr class="normTxt <?=$c?>" >
