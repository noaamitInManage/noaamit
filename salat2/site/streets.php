...
include_once($_project_server_path.'_static/cities.'.$langSalat.'.inc.php'); //$citiesArr
asort($citiesArr);

$_Proccess_Main_DB_Table = "tb_streets";
 ...
 */
$_Proccess_FW_Params = array('lang_id','city_id');
...
function module_updateStaticFiles() {
    ...
    if($_Proccess_Has_MultiLangs){
        $UpdateStatic = new streetsLangsUpdateStaticFiles();
        $UpdateStatic->updateStatics();
    } else{
        $UpdateStatic = new streetsUpdateStaticFiles();
        $UpdateStatic->updateStatics();
    }
}
