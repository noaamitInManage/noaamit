...
$_Proccess_Main_DB_Table = "tb_301";
...
if($act=='new'){
	...
	$file_path = $_FILES["list"]["tmp_name"];
	if(!empty($_FILES["list"])){

		$ext = explode(".", $_FILES["list"]["name"]);
		...
		if (($handle = fopen($file_path, "r")) !== FALSE) {
			while (($itemsArr = fgetcsv($handle, 1024, ",")) !== FALSE) {


				$post_city_id = intval($post_city_id);
				$db_fields = array(
					'from_url' =>  $itemsArr[0],
					'to_url' =>  $itemsArr[1],
					'active' => 1,
				...
				$query = "INSERT INTO `{$_Proccess_Main_DB_Table}` (`" . implode("`,`", array_keys($db_fields)) . "`) VALUES ('" . implode("','", array_values($db_fields)) . "')";
				$Db->query($query);
			}
		}else{
			return 3;
...
function module_updateStaticFiles() {
	...
	$UpdateStatic = new pages301UpdateStaticFiles();
...
<head>
	...
	<link rel="StyleSheet" href="../_public/ltr.css" type="text/css">
...
<div class="" style="width: 100%; padding: 10px; position: relative;">

	<br/><br/><br/>
	<form action="" method="POST" enctype="multipart/form-data">
		<input name="act" type="hidden" value="upload_file" />
		<label style="font-size: 15px; font-weight: bold;">
			* יש לעלות קובץ csv:<br>
			<span style="font-size: 13px; font-weight: normal">
				from_url_1,to_url_1<br>
				from_url_2,to_url_2
			</span>
		</label>
		<br/>
		<input name="list" type="file" />
		<input class="buttons" type="submit" value="בצע העלאה" />
	</form>
</div>
				...
				<table cellpadding="3" cellspacing="0" align="center"><tr>
						...
						foreach ($fieldsArr AS $key=>$fieldArr) {
							if($fieldArr['input']['searchable']==true){
								print "<td>".draw_genric_search($fieldArr,$key) .":";
								print $fieldArr['title']."</td>";
							}
						}
						?>
