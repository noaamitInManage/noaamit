...
elseif ($act == "del") {
    ...
    if ($_Proccess_Has_MultiLangs) {
        ...
        @unlink($_SERVER['DOCUMENT_ROOT'] . '/_static/stores/' . get_item_dir($obj_id) . '/' . $languagesArr[$module_lang_id]['title'] . '/tmpl-' . $obj_id . '.inc.php');
    } else {
        $query = "DELETE FROM {$_Proccess_Main_DB_Table} WHERE id='{$obj_id}'";
        // delete static file !!!
        @unlink($_SERVER['DOCUMENT_ROOT'] . '/_static/stores/' . get_item_dir($obj_id) . '/store-' . $obj_id . '.inc.php');
    }
    $result = $Db->query($query);
