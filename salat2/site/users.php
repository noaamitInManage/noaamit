...
//include($_project_server_path.$_includes_path.'/class/users.class.inc.php');
include_once($_SERVER['DOCUMENT_ROOT'] . '/index.class.include.php');
include_once($_SERVER['DOCUMENT_ROOT'] . '_inc/class/Enums/UsersTypesEnum.enum.inc.php');

$_Proccess_Main_DB_Table = "tb_users";
...
//die('<hr /><pre>' . print_r($_REQUEST, true) . '</pre><hr />');
    ...
    if ($obj_id) {  // ### UPDATE QUERY ###
        ...
        if (isset($_REQUEST['password']) && trim($_REQUEST['password'])) {
            User::change_password($obj_id, $_REQUEST['password']);
        }
    ...
    if (isset($_FILES['picture']['tmp_name']) && ($_FILES['picture']['tmp_name'])) {
        ...
        $image_url = User::$profile_picture_path . '/' . $image_name . '.' . $ext;
...
//	module_updateStaticFiles();
    ...
    if ($_Proccess_Has_MultiLangs) {
        ...
        @unlink($_SERVER['DOCUMENT_ROOT'] . '/_static/users/' . get_item_dir($obj_id) . '/' . $languagesArr[$module_lang_id]['title'] . '/user-' . $obj_id . '.inc.php');
    } else {
        $query = "DELETE FROM {$_Proccess_Main_DB_Table} WHERE id='{$obj_id}'";
        // delete static file
        @unlink($_SERVER['DOCUMENT_ROOT'] . '/_static/users/' . get_item_dir($obj_id) . '/user-' . $obj_id . '.inc.php');
    }
    $result = $Db->query($query);
    ...
    if (isset($_REQUEST['csv']) && ($_REQUEST['csv'])) {
        ...
        $str = implode(',', $query_fieldsArr);
        $query = "
			SELECT {$str}
			 FROM `tb_users`";
        query_to_csv($query, 'user_log' . date('d_m_Y') . '.csv', true);
        exit();
    }
    $resultArr['result'] = $Db->query($query);

}

function loadStatus(){
	global $row;
    $user_typeArr = UsersTypesEnum::$typesTranslationsArr;

    $html='';
	foreach ($user_typeArr AS $key => $value){
		if( in_array($key,array(1, 2)) && !in_array($_SERVER['REMOTE_ADDR'],configManager::$familiar_ipsArr)) {
            continue;
		}
		$checked= ($row['user_type']&$key) ? 'checked="checked"' : '';
		$html.='<label><input type="checkbox" value="'.$key.'" name="_status_[]" '.$checked.' />'.$value.' </label> <br />';
	}
	return $html;
}

function saveStatus(){
	return array_sum($_REQUEST['_status_']);
}
...
function show_user_token($user_id){

	$Db = Database::getInstance();

$html=<<<HTML
	<div id="user_token_wrapper">
...
HTML;
    ...
    $query="SELECT `token_device`, `user_id`, `device_type`
                FROM `tb_push__device`
...
function draw_base64_image($value){
    if($value){
        return <<< HTML
        <img src="data:image/png;base64,{$value}" width="100"/>
HTML;
    }
    return "";
}
...
<head>
	...
	<link rel="StyleSheet" href="../_public/sweet-alert/sweetalert.css" type="text/css">
	...
	<script type="text/javascript" src="../_public/sweet-alert/sweetalert.min.js"></script>
...
$(function () {
    ...
    $("#show_x_btn").on('click', function (event) {
        var num = window.prompt("How many users to show?", "50");
        if (num) {
            window.location.href = '?act=show&<?=$fwParams;?>&limit=' + num;
        }
    });
	...
	<style type="text/css">
		...
		.large_table th, .large_table td .small{
			width:30px;
		}
		.large_table th, .large_table td .large{
			width:200px;
		}
		.large_table{
...
<input type="button" class="buttons" onclick="javascript: location.href='?act=show<?php echo $fwParams;?>';" value="<?=$_LANG['BTN_SHOW_ALL'];?>" />
	<input type="button" class="buttons" id="show_x_btn" <? /*onclick="javascript: location.href='?act=show&last_100=true<?php echo $fwParams;?>';"*/?> value="Show X Last Users" />
	<a href="?act=show<?=$fwParams;?>&csv=true" >
