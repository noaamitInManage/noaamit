...
$_Proccess_Main_DB_Table = "tb_side_menu";
...
if($act=='new'){
    if($obj_id){
        ...
        if($_REQUEST['tab_barError'] > 0){
            echo "<script>alert('יש לבחור במקסימום 3 קישורים לתפריט תחתון');</script>";
        }
        if($_Proccess_Has_MultiLangs){
...
//die('<hr /><pre>' . print_r($_REQUEST, true) . '</pre><hr />');
    ...
    if($obj_id){  // ### UPDATE QUERY ###
        ...
        check_tab_bar_limit($_REQUEST['tab_bar']);
    ...
    if($_Proccess_Has_MultiLangs){
        ...
        @unlink($_SERVER['DOCUMENT_ROOT'].'/_static/menu/'.get_item_dir($obj_id).'/'.$languagesArr[$module_lang_id]['title'].'/menu-'.$obj_id.'.inc.php');
    }else{
        $query="DELETE FROM {$_Proccess_Main_DB_Table} WHERE id='{$obj_id}'";
        // delete static file !!!
        @unlink($_SERVER['DOCUMENT_ROOT'].'/_static/menu/'.get_item_dir($obj_id).'/menu-'.$obj_id.'.inc.php');
    }
    $result = $Db->query($query);
...
function check_tab_bar_limit($tab_bar){
    global $fwParams, $_Proccess_Main_DB_Table;

    $Db = Database::getInstance();

    if($tab_bar == 1) {
        $query = "SELECT count(id) AS cnt FROM {$_Proccess_Main_DB_Table} WHERE `id` <> '{$_REQUEST['inner_id']}' AND `tab_bar` = 1";
        $result = $Db->query($query);
        $row = $Db->get_stream($result);

        if ($row['cnt'] > 2) {
            header("Location: ?act=new&id=" . $_REQUEST['inner_id'] . $fwParams . "&tab_barError=" . $row['cnt']);
            exit();
        }
    }
}
function module_updateStaticFiles() {
    ...
    if($_Proccess_Has_MultiLangs){
        $UpdateStatic = new menuLangsUpdateStaticFiles();
        $UpdateStatic->updateStatics();
    }else{
        $UpdateStatic = new menuUpdateStaticFiles();
        $UpdateStatic->updateStatics();
    }
