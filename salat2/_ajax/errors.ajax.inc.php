...
//		$rawValue = $Db->make_escape(rawurldecode($item['text']));
            ...
            $query = "UPDATE `tb_errors` SET `content`='{$rawValue}' ,`descripation`='{$descripation}' WHERE `error_code`='{$key_code}' AND `lang_id`='{$lang_id}'";
        ...
        if ($updateRes != false) {
            ...
            $query = "INSERT INTO `tb_errors` (`" . implode("`,`", array_keys($db_fields)) . "`) VALUES ('" . implode("','", array_values($db_fields)) . "')";
            $res = $Db->query($query);
            if ($res != false) {
                $status = 'ok';
            } else {
    ...
    case "delete":
        ...
        $deleteSql = "DELETE FROM `tb_errors` WHERE `error_code` = '{$rawValue}'";
    ...
    case "save-new":
        ...
        if (!$hasIllegalChars) {
            ...
            $keyCodeSql = "SELECT * FROM `tb_errors` WHERE `error_code`='{$key_code}'";
...
//				$rawValue = $Db->make_escape(sanitizeText(rawurldecode($value['text'])));
                ...
                $newKeySql = "INSERT INTO `tb_errors` (lang_id,error_code,content) VALUES " . implode(',', $values);
