...
include_once($_project_server_path . '/_static/banners-slots.inc.php'); //$bannersSlotsArr
include_once($_project_server_path . '/_static/campaigns.inc.php'); //$campaignsArr

$_Proccess_Main_DB_Table = "tb_banners";
...
if ($act == 'new') {
    if ($obj_id) {
        ...
        // hide zero value fields for new items
        $fieldsArr['total_views']['value']['new']['source'] = 'none';
        $fieldsArr['total_views']['value']['new']['details'] = '';
        $fieldsArr['total_clicks']['value']['new']['source'] = 'none';
        $fieldsArr['total_clicks']['value']['new']['details'] = '';
        $fieldsArr['last_update']['value']['new']['source'] = 'none';
        $fieldsArr['last_update']['value']['new']['details'] = '';
...
//die('<hr /><pre>' . print_r($_REQUEST, true) . '</pre><hr />');
    ...
    $module_table = $_Proccess_Main_DB_Table;
    $whereClause = "`id` = '{$_REQUEST['inner_id']}'";
    if ($_Proccess_Has_MultiLangs) {
        $whereClause = "`obj_id` = '{$_REQUEST['inner_id']}' AND `lang_id`='{$module_lang_id}'";
        $module_table .= '_lang';
    }

    $imageFields = array('flash');  /* all images fields needs to be in this array */
    foreach ($imageFields as $field) {
        if ($_FILES[$field . '_ext']['tmp_name']) {
            ...
            if (in_array($ext, array('swf'))) {
				...
				";
                $Db->query($query);
                $targetFile = $dir . $_REQUEST['inner_id'] . '.' . $ext;
            }
        }
...
function module_updateStaticFiles()
{
    ...
    if ($_Proccess_Has_MultiLangs) {
        $UpdateStatic = new bannersLangsUpdateStaticFiles();
        $UpdateStatic->updateStatics();
    } else {
        $UpdateStatic = new bannersUpdateStaticFiles();
        $UpdateStatic->updateStatics();
    }
