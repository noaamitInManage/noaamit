...
$_Proccess_Main_DB_Table = "tb_cg_errors";
...
//die('<hr /><pre>' . print_r($_REQUEST, true) . '</pre><hr />');
	...
	$query  = "SELECT * FROM {$_Proccess_Main_DB_Table} {$where} ORDER BY error_code";
...
while($item = $Db->get_stream($itemsRes)){
	if (!is_array($items[$item['error_code']])){ $items[$item['error_code']] = array(); }
...
<head>
	...
	<script type="text/javascript" src="../_public/cg_errors.js"></script>
...
<div class="maindiv">
	...
	// build table headers
	$tableHeader = '<tr class="dottTbl"><td>קוד שגיאה</td>';
...
newRow;
		foreach ($langs as $key => $value) {
			$directionClass = ( $value['direction'] == 1? 'ltr-input' : 'rtl-input' );
		...
		<input type="button" id="saveNewError" class="buttons" value="הוסף">
		<input type="button" id="cancelNewError" class="buttons red" value="ביטול">
		</td>
	</tr>
newRow;
		echo $newTranslation;
...
<td> <span>תיאור</span></td>
						<td>
						 <input type="text" rel="descripation_$key_code" data-langid="{$key}" data-desc="1"  class=" {$directionClass}"  value="{$value['descripation']}"/>
</td>
HTML;
...
tableRow;
			...
			$tableRow .= '<td width="'.$columnWidth.'%"><input type="button" class="buttons saveError" value="שמור" ><input type="button" class="buttons red deleteError" value="'.$_LANG['BTN_DEL'].'" ></td>';
			$tableRow .= '<tr>';
			echo $tableRow;
		}
		?>
