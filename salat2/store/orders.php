...
$_Proccess_Main_DB_Table = "tb_orders";
...
// fetch all agencies 
$agenciesArr = array('-- Choose --');
$agenciesSql = "SELECT id,contact_person FROM `tb_agents`";
$agenciesRes = $Db->query($agenciesSql);
while($agent = $Db->get_stream($agenciesRes)){
	$agenciesArr[$agent['id']] = $agent['contact_person'];
}
...
if($act=='new'){
	if($obj_id){
		...
		// start & end dates set as today for new object 
		$fieldsArr['start_date']['value']['new']['details'] = date('d-m-Y',time());
		$fieldsArr['end_date']['value']['new']['details'] = date('d-m-Y',time());
...
//die('<hr /><pre>' . print_r($_REQUEST, true) . '</pre><hr />');
	...
	if($_Proccess_Has_MultiLangs){
		...
		@unlink($_SERVER['DOCUMENT_ROOT'].'/_static/orders/'.get_item_dir($obj_id).'/'.$languagesArr[$module_lang_id]['title'].'/order-'.$obj_id.'.inc.php');
	}else{
		$query="DELETE FROM {$_Proccess_Main_DB_Table} WHERE id='{$obj_id}'";
		// delete static file
		@unlink($_SERVER['DOCUMENT_ROOT'].'/_static/orders/'.get_item_dir($obj_id).'/order-'.$obj_id.'.inc.php');
	}
	$result = $Db->query($query);
...
//	if($_Proccess_Has_MultiLangs){
//		$UpdateStatic = new eventsLangsUpdateStaticFiles();
...
function buildOrderItems($order_id){
	global $productTypeArr;

	$Db = Database::getInstance();

	$html = <<<orderItemsHeader
<table id="orderItems" border="0" cellpadding="8">
	<tr>
		<th>סוג פריט</th>
		<th>קוד פריט</th>
		<th>מידע</th>
...
orderItemsHeader;
	$index = 0;
	$orderItemsSql = "SELECT * FROM `tb_order_items` WHERE `order_id`={$order_id}";
	$orderItemsRes = $Db->query($orderItemsSql);
	while($item = $Db->get_stream($orderItemsRes)){
		$rowClass = $index % 2 == 0? 'row-odd' : 'row-even';
		$html .= <<<orderItems
	<tr class="{$rowClass}">
		<td>{$productTypeArr[$item['product_type']]}</td>
		<td>{$item['id']}</td>
		<td>{$item['information']}</td>
		<td>\${$item['commision_sum']}</td>
		<td>\${$item['sum']}</td>
	</tr>
orderItems;
		$index++;
...
/*
	...
	INSERT INTO  `c19_gensis`.`tb_orders` (
		`id` ,
		`agent_id` ,
		`payment_type` ,
		`commission_sum` ,
	...
	VALUES (
		NULL ,  '17',  '1',  '0',  '0',  '',  '0',  '1356962930',  '1356962927'
	);
