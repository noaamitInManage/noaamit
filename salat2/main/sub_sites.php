...
include_once($_project_server_path . $_includes_path . "site.array.inc.php");
...
include_once($_SERVER['DOCUMENT_ROOT'] . '/_static/languages.inc.php'); // $languagesArr
...
if ($act == 'new') {
    if ($obj_id) {
        ...
        $query = "SELECT lang_id FROM tb_sub_sites_lang_link WHERE site_id='{$row['id']}'";
        $result_lang = $Db->query($query);
        while ($row_lang = $Db->get_stream($result_lang)) {
            $row['langs_' . $row_lang['lang_id']] = $row_lang['lang_id'];
        }
    } else {
        $submit = "הוסף";
    }
    ...
    $Db->query("DELETE FROM tb_sub_sites_lang_link WHERE site_id = '{$_REQUEST['inner_id']}'");
    foreach ($_REQUEST['langs'] as $langID) {
        $Db->query("INSERT INTO tb_sub_sites_lang_link SET site_id='{$_REQUEST['inner_id']}', lang_id='{$langID}'");
    }
...
function module_updateStaticFiles()
{
    ...
    $UpdateStatic = new subSitesUpdateStaticFiles();
    $UpdateStatic->updateStatics();
    $UpdateStatic->updateAllStaticsFiles();
}
            ...
            <?php for ($count = $resultArr['result']->num_rows, $i = 0; $row = $Db->get_stream($resultArr['result']); $i++) { ?>
                <tr class="normTxt">
                    ...
                    <td class="dottTblS">
                        ...
                        <?php }
                        if (false && !in_array($row['id'], $_Proccess_HC_RowsID_Arr_NOT_DELETABLE)) { ?>
