...
<div class="maindiv">
    ...
    <? if ($_Proccess_Has_MultiLangs) { ?>
        <div class="tabs_con">
            <input type="button"
                   class=" tabs <?= ((isset($_REQUEST['cache'])) || ($_REQUEST['cache'] == 'opCache')) ? 'active' : ''; ?>"
                   onclick="javascript: $('#iframe_holder').attr('src','opcache.php'); $('.tabs').removeClass('active'); $(this).addClass('active');"
                   value="opCache">&nbsp;
            <input type="button"
                   class=" tabs <?= ((!isset($_REQUEST['cache'])) || ($_REQUEST['cache'] == 'Memcached')) ? 'active' : ''; ?>"
                   onclick="javascript:  $('#iframe_holder').attr('src','/memcached'); $('.tabs').removeClass('active'); $(this).addClass('active');"
                   value="Memcached">
        </div>
    <? } ?>
    <?php if ($act == "show") { ?>

        <iframe id="iframe_holder" src="/memcached" width="100%" height="100%" frameborder="0">
