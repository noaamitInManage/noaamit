...
include_once($_project_server_path . $_salat_path . $_includes_path . 'moduleUpdateStaticFiles.php'); // 15/04/2011
...
$_Proccess_Main_DB_Table = "tb_csp";
...
$sitesNameArr = array();
foreach ($sitesArr as $key => $siteArr) {
    $sitesNameArr[$key] = $siteArr['name'];
}
...
//die('<hr /><pre>' . print_r($_REQUEST, true) . '</pre><hr />');
    ...
    if (isset($_FILES['picture']['tmp_name']) && ($_FILES['picture']['tmp_name'])) {
        ...
        $image_url = companiesManager::$profile_picture_path . '/' . $image_name . '.' . $ext;
    ...
    include($_SERVER['DOCUMENT_ROOT'] . '/_inc/csp.inc.php');
    $testArr = $script_white_list_default;
    for ($i = 0; $i < count($script_white_list_default); $i++) {
        $testArr[$i] = [id => $i + 1, url => $testArr[$i]];
    }
}
        ...
        $(function () {
            ...
            $('#company_industries').on({
                keydown: function () {
                    ...
                    if (!parseInt(obj_id)) {
                        swal('', 'יש לבצע שמירה לפני שיוך התגית לחברה', 'error');
                        $(this).val('');
                    }
                },
                click: function () {
            ...
            $('body').on('click', '.add_connected_industry', function () {
                var company_id = parseInt("<?=$obj_id?>");
                var industry_id = parseInt($(this).parent().parent().find('#company_industries').attr('current_industry_id'));
                if (company_id > 0 && industry_id > 0) {
                    $.post('/salat2/_ajax/ajax.index.php', {
                            'company_id': company_id,
                            'industry_id': industry_id,
                            'file': 'company',
                            'action': 'add_company_industry',
                        },
                        function (response) {
                            if (response.err != "") {
                                ...
                                $('.dynamic_industries').html(response.html);
                                industry_ac();
                            }
                        }, "json"
                    );
            ...
            $('body').on('click', '.delete_connected_industry', function () {
                $.post('/salat2/_ajax/ajax.index.php', {
                        'company_id': $(this).attr('company_id'),
                        'industry_id': $(this).attr('industry_id'),
                        'file': 'company',
                        'action': 'delete_company_industry',
                    },
                    function (response) {
            ...
            function industry_ac(sameCategory = false) {
                var obj_id = '<?=$obj_id?>';
                if ((obj_id !== undefined) && (obj_id > 0)) {

                    $('#company_industries').off();
                    $('#company_industries').autocomplete('/salat2/_ajax/ajax.index.php', {
                        'extraParams': {
                            'company_id': obj_id,
                            'file': 'auto_complete/company',
                            'action': 'get_company_industries',
                        }
                    }).result(function (event, data) {
                        var current_industry_id = parseInt(data);
                        $(this).attr('current_industry_id', current_industry_id);
                    });
                }
            }

            /* --- END Company Industries --- */
        });
    </script>
    <style type="text/css">
        /* auto media load */
...
<div class="maindiv">
    ...
    <?php if ($act == "show") { ?>
        <table width="100%" border="0" cellpadding="3" cellspacing="1" style="empty-cells:show;" class="table-list">
            ...
            <?php for ($i = 0; $i < count($testArr); $i++) {
                $c = ($i % 2 == 0) ? 'even' : 'odd'; ?>
                <tr class="normTxt <?= $c ?>">
                    <?php $columns_count = fields_get_show_rows_fields($fieldsArr, $testArr[$i], false); ?>
                    <?php if ($_Proccess_Has_Ordering_Action) { ?>
                        <td class="dottTblS"><?php echo outputOrderingArrows($count, $i, 'id', $testArr[$i]['id'], $testArr[$i]['order_num']); ?></td>
                    <?php } ?>
                </tr>
            <?php }
            if ($resultArr['paging']) { ?>
