...
$_Proccess_Main_DB_Table = "tb_general_settings";
...
//eval("mediaSelector('media_id',\$media_is_resulationArr,array(1,0));");
...
function module_updateStaticFiles()
{
    ...
    if ($_Proccess_Has_MultiLangs) {
        $UpdateStatic = new generalSettingsLangsUpdateStaticFiles();
        $UpdateStatic->updateStatics();
    } else {
        $UpdateStatic = new generalSettingsUpdateStaticFiles();
        $UpdateStatic->updateStatics();
    }
...
<head>
    ...
    <script type="text/javascript">
        var res_sizeArr =<?=json_encode($media_is_resulationArr);?>;
    ...
    <?php if ($act == "show") { ?>
        <table width="100%" border="0" cellpadding="3" cellspacing="1" style="empty-cells:show;" class="table-list">
            ...
            <?php for ($count = $resultArr['result']->num_rows, $i = 0; $row = $Db->get_stream($resultArr['result']); $i++) {
                $c = ($i % 2 == 0) ? 'even' : 'odd'; ?>
                <tr class="normTxt <?= $c ?>" onmouseover="this.className='normTxt hover <?= $c ?>';"
                    onmouseout="this.className='normTxt <?= $c ?>';">
