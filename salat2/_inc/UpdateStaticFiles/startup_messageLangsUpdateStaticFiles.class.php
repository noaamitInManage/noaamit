...
class startup_messageLangsUpdateStaticFiles extends moduleUpdateStaticFiles  implements iUpdate{
	...
	function __construct(){
		parent::__construct();
		$this->_Proccess_Main_DB_Table = 'tb_startup_message';
		$this->_ProcessID = 77;
		$this->className= trim(get_class());
		$this->file_name='startup_messages.inc.php';
		$this->itemsArr_name='startup_messagesArr';
		$this->name='הודעות פתיחה (מובייל)';
	}
	...
	function updateStatics($id=''){
		...
		$smart_dir=parent::smartLangDirctory('/_static/startup_message/',$_REQUEST['inner_id'],$module_lang_id);
		@unlink($_SERVER['DOCUMENT_ROOT'].'/_static/startup_message.'.$languagesArr[$module_lang_id]['title'].'.inc.php');
		...
		updateStaticFile("SELECT Main.id,Lang.title FROM `{$this->_Proccess_Main_DB_Table}` AS Main
						         	LEFT JOIN `{$this->_Proccess_Main_DB_Table}_lang` AS Lang ON (
						               			Main.`id`=Lang.`obj_id`
						               		) WHERE Lang.lang_id={$module_lang_id} AND
						               				 	Main.`active`=1
         	...
         	",
			'/_static/startup_messages.'.$languagesArr[$module_lang_id]['title'].'.inc.php',
			'startup_messagesArr','id',true,true);


		if($_REQUEST['inner_id']) {
			@unlink($_SERVER['DOCUMENT_ROOT'].$smart_dir.'startup_message-'.$_REQUEST['inner_id'].'.inc.php');
			...
			updateStaticFile("SELECT * FROM `{$this->_Proccess_Main_DB_Table}` AS Main
				...
				$smart_dir.'startup_message-'.$_REQUEST['inner_id'].'.inc.php',
				'startup_messageArr');
		}
	}
