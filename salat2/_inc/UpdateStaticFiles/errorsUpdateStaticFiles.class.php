...
class errorsUpdateStaticFiles extends moduleUpdateStaticFiles implements iUpdate
{
    ...
    {
        parent::__construct();
        $this->_Proccess_Main_DB_Table = 'tb_errors';
        $this->_ProcessID = 80;
        $this->className = trim(get_class());
        $this->file_name = 'errors.inc.php';
        $this->itemsArr_name = 'errorsArr';
        $this->name = 'שגיאות';
    }
    ...
    function updateStatics($id = '')
    {
        ...
        foreach ($languagesArr as $lang_id => $lang) {
            @unlink($_SERVER['DOCUMENT_ROOT'] . '/_static/errors.' . $lang['title'] . '.inc.php');
            updateStaticFile("SELECT error_code,content FROM `{$this->_Proccess_Main_DB_Table}` WHERE `lang_id`={$lang_id}",
                '/_static/errors.' . $lang['title'] . '.inc.php',
                'errorsArr', 'error_code', true);
        }
