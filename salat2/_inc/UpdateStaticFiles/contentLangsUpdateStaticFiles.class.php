...
class contentLangsUpdateStaticFiles extends moduleUpdateStaticFiles  implements iUpdate{
    ...
    function updateStatics($id=''){
        ...
        $smart_dir=parent::smartLangDirctory('/_static/contents/',$_REQUEST['inner_id'],$module_lang_id);
        @unlink($_SERVER['DOCUMENT_ROOT'].'/_static/contents.'.$languagesArr[$module_lang_id]['title'].'.inc.php');
        ...
        updateStaticFile("SELECT Main.id,Lang.title,Main.order_num FROM `{$this->_Proccess_Main_DB_Table}` AS Main
         	...
         	",
            '/_static/contents.'.$languagesArr[$module_lang_id]['title'].'.inc.php',
            'contentsArr');
