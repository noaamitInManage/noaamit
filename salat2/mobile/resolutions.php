...
$_Proccess_Main_DB_Table = "tb_resolutions";
...
if($act=='zip'){
	$answ='';
	$path=$_SERVER['DOCUMENT_ROOT'].'/_static/media_zip/';
	if(isset($_REQUEST['all'])&&$_REQUEST['all']==1){
		$ResolutionObject=new resolutionsManager();
		$answ=$ResolutionObject->make_all_resolutions($path);
	}else if(isset($_REQUEST['res_id'])&&$_REQUEST['res_id']){
		$ResolutionObject=new resolutionsManager($_REQUEST['res_id']);
		$answ=$ResolutionObject->make_all_zips($path,$_REQUEST['res_id']);
	}else if(isset($_REQUEST['all'])&&$_REQUEST['all']==2){
		$ResolutionObject=new resolutionsManager();
		$answ=$ResolutionObject->make_pc_zip($path);
	}
	if($answ=="OK"){
		header ("Location: ?act=show&zipok=1".$fwParams);
	}else{
		header ("Location: ?act=show&zipok=2".$fwParams);
	}
}
...
if($act=='new'){
	...
	if (isset($_REQUEST['file'])) {
		$filezip = $_SERVER['DOCUMENT_ROOT'].'/_static/media_zip/'.$_REQUEST['file'].'.zip';
		if (file_exists($filezip) && is_readable($filezip)) {
...
//die('<hr /><pre>' . print_r($_REQUEST, true) . '</pre><hr />');
	...
	$query="DELETE FROM tb_media_resolutions WHERE resolution_id='{$obj_id}'";
	$result = $Db->query($query);
...
function module_updateStaticFiles() {
	...
	if($_Proccess_Has_MultiLangs){
		//$UpdateStatic = new resolutionsLangUpdateStaticFiles();
		//$UpdateStatic->updateStatics();
	}else{
		$UpdateStatic = new resolutionsUpdateStaticFiles();
		$UpdateStatic->updateStatics();
	}
}
...
<head>
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			//size of file
			$.each($('.file_size'),function(key,val){
				if($(val).html()=="0kb"){
					$(val).css('color','red');
				}else{
					$(val).css('color','black');
...
<input type="button" class="buttons" value="צור גרסת מדיה לכל הרזוליוציות" onclick="javascript: location.href='?act=zip&all=1';" /> &nbsp;
<input type="button" class="buttons" value="צור קובץ מרכזי עבור PC" onclick="javascript: location.href='?act=zip&all=2';" /> &nbsp;
<?if(isset($_REQUEST['zipok'])&&$_REQUEST['zipok']==1){?>
	<div>קובץ נשמר בהצלחה!</div>
<?}else if(isset($_REQUEST['zipok'])&&$_REQUEST['zipok']==2){?>
	<div>שגיא! הקובץ לא נשמר, תנסה שנית.</div>
<?}?>
<div class="maindiv">
	<br/>
	<? if($_Proccess_Has_GenricSearch){ ?>
			...
			<?php  for($count = $resultArr['result']->num_rows,$i=0;$row = $Db->get_stream($resultArr['result']);$i++){
				...
				<tr class="normTxt <?=$c?>" >
					...
					<td class="dottTblS" style="text-align: center;">
						<input type="button" class="buttons" value="צור גרסת מדיה" onclick="javascript: location.href='?act=zip&res_id=<?php echo $row['id'].$fwParams;?>';" /> &nbsp;
						<div><?=date('d.m.Y',$row['last_update']);?></div>
						<div><a href="#" onclick="javascript:document.location.href='?act=download&file=<?=$row['title'];?>'" >download file</a></div>
						<?clearstatcache();?>
						<div class="file_size"><?=(filesize($_SERVER['DOCUMENT_ROOT'].'/_static/media_zip/'.$row['title'].'.zip'))?floor((filesize($_SERVER['DOCUMENT_ROOT'].'/_static/media_zip/'.$row['title'].'.zip')/1000)):'0';?>kb</div>
					</td>
				</tr>
