...
global $gd_images_typesArr, $gd_images_typesKeywordsArr;

$_Proccess_Main_DB_Table = "tb_gd_images";
...
$config_gd_images_typesKeywordsArr = configManager::$gd_images_typesKeywordsArr;
$config_gd_images_typesArr = configManager::$gd_images_typesArr;
...
if ($act == 'new') {
    ...
    if (isset($_REQUEST["api_key"])) {
        $_REQUEST["api_key"] = str_replace(" ", "_", $_REQUEST["api_key"]);
    }
    ...
    foreach ($resolutionsArr as $resolutionArr) {
        $uploaded_name = "uploadResolution_" . $resolutionArr["id"];
        $image_path = $dir . $_REQUEST['inner_id'] . '/' . $resolutionArr["id"];
        if ($_FILES[$uploaded_name]['tmp_name']) {
            $nameArr = explode('.', $_FILES[$uploaded_name]['name']);
            $ext = strtolower(end($nameArr));

            if (in_array($ext, array('png', 'jpg', 'jpeg'))) {
                if (!is_dir($dir . $_REQUEST['inner_id'])) {
                    @mkdir($dir . $_REQUEST['inner_id'], 0777);// crate a new dir
                    chmod($dir . $_REQUEST['inner_id'], 0777);// if the first line didnt Success
                }

                $image_pattern = $image_path . ".{png,jpg,jpeg}";
                $imagesArr = glob($image_pattern, GLOB_BRACE);
                foreach ($imagesArr as $image_full_path) {
                    @unlink($image_full_path);
                }

                move_uploaded_file($_FILES[$uploaded_name]['tmp_name'], $image_path . '.' . $ext);
            }
        }
    }
    ...
    if ($_Proccess_Has_MultiLangs) {
        ...
        //@unlink($_SERVER['DOCUMENT_ROOT'].'/_static/gd_images/'.get_item_dir($obj_id).'/'.$languagesArr[$module_lang_id]['title'].'/gd_image-'.$obj_id.'.inc.php');
    } else {
        $query = "DELETE FROM {$_Proccess_Main_DB_Table} WHERE id='{$obj_id}'";
        // delete static file !!!
        //@unlink($_SERVER['DOCUMENT_ROOT'].'/_static/gd_images/'.get_item_dir($obj_id).'/gd_image-'.$obj_id.'.inc.php');
    }
    $result = $Db->query($query);
...
function module_updateStaticFiles()
{
    ...
    if ($_Proccess_Has_MultiLangs) {
        $UpdateStatic = new gd_imagesLangsUpdateStaticFiles();
        $UpdateStatic->updateStatics();
    } else {
        $UpdateStatic = new gd_imagesUpdateStaticFiles();
        $UpdateStatic->updateStatics();
    }

}

function createUploadImagesResolutions()
{
    global $resolutionsArr, $row;

    $itemsHTML = '';
    foreach ($resolutionsArr AS $resolutionArr) {
        $itemsHTML .= '<div class="dynamicItemsCont_' . $resolutionArr["id"] . '">';
        $itemsHTML .= '<label style="font-weight: bold; color: darkblue; text-decoration: underline;" >העלאת תמונות עבור: ' . $resolutionArr["title"] . '</label><br/>';
        $itemsHTML .= '<input type="file" class="buttons orange" name="uploadResolution_' . $resolutionArr["id"] . '" /> &nbsp;';

        $image_title = $row['api_key'] . '_' . $resolutionArr["title"];
        $image_partial_path = '/_media/gd_images/' . $row['id'] . '/' . $resolutionArr["id"];
        $image_pathArr = glob($_SERVER['DOCUMENT_ROOT'] . $image_partial_path . ".{png,jpg,jpeg}", GLOB_BRACE);

        if ($image_pathArr !== false && !empty($image_pathArr)) {
            $image_path = $image_pathArr[0];
            $imageArr = explode(".", $image_path);
            $ext = end($imageArr);
            $image_path = $image_partial_path . '.' . $ext;
            $itemsHTML .= '<div>
                                <a style="padding: 10px; cursor: pointer;" href="' . $image_path . '" target="_blank">
                                    <img style="width:100px; height: 50px;" src="' . $image_path . '" title="' . $resolutionArr["title"] . '">
                                </a>
                            </div>';
        }
        $itemsHTML .= '</div>';
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("input[name='type']").live("change", function () {
                if ($("input[type=radio][name=type]:checked").val() == <?=$config_gd_images_typesKeywordsArr["regular"]?>) {
                    $(".regular_section").closest("tr").show();
                    $(".full_screen_section").closest("tr").hide();
                } else {
                    $(".regular_section").closest("tr").hide();
                    $(".full_screen_section").closest("tr").show();
                }
            });
