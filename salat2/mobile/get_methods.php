...
include_once($_project_server_path . $_includes_path . 'class/api/ApiResponse.class.inc.php');
include_once($_project_server_path . $_includes_path . 'class/api/SendsApiResponses.inc.php');
include_once($_project_server_path . $_includes_path . 'class/api/ApiHelpers.inc.php');
include_once($_project_server_path . $_includes_path . 'class/api/ApiExceptionHandler.class.inc.php');
include_once($_project_server_path . $_includes_path . 'class/BaseManager.class.inc.php');//BaseManager

$_Methods_To_Exclude = array(
        '__construct',
        '__destruct',
        '__set',
        '__get',
...
//die('<hr /><pre>' . print_r($_REQUEST, true) . '</pre><hr />');
	...
	$imagesArr = array('image-iphone5','image-iphone4','image-iphone3','image-andriod','image-andriod-hdpi','image-andriod-xhdpi','image-andriod-tablet');
	foreach( $imagesArr AS $image_name){
		if($_FILES[$image_name]['tmp_name']){
			$ext = strtolower(end(explode('.',$_FILES[$image_name]['name'])));
			if(in_array($ext, array('png'))){
				move_uploaded_file($_FILES[$image_name]['tmp_name'], $dir.$_REQUEST['inner_id'].str_replace('image-','-',$image_name).'.'.$ext);
			}
		}

		if(isset($_REQUEST[$image_name.'_delete'])){
			@unlink($dir.$_REQUEST['inner_id'].'-'.str_replace('image-','',$image_name).'.png');
		}
	}
...
//	$resultArr = getSqlPagingArr($query);
    ...
    if($_POST['update_get_methods']){
        update_get_methods();
    }

    $query = "SELECT * FROM {$_Proccess_Main_DB_Table}";
    $results = $Db->query($query) or db_showError(__FILE__, __LINE__, $query);
    while ($row = $Db->get_stream($results)){
        $resultsArr[] = $row;
    }

    include_once($_project_server_path.$_includes_path."class/BaseManager.class.inc.php");
    include_once($_project_server_path.$_includes_path."class/apiManager.class.inc.php");
    $class = new ReflectionClass(apiManager::class);
    $methods = $class->getMethods();
    $rowsArr = methods_active($methods, $resultsArr);
}

function search_for_method_name($method_name, $array){
    foreach ($array as $arr){
        if($arr['method_name'] === $method_name && $arr['active'] === '1'){
            return $arr['id'];
        }
    }
    return false;
...
function methods_active($methodsArr, $resultsArr){
    global $_Methods_To_Exclude;
    $returnArr = array();
    foreach ($methodsArr as $id => $methodArr) {
        if(in_array($methodArr->name, $_Methods_To_Exclude)){
            continue;
        }
        if(search_for_method_name($methodArr->name, $resultsArr)) {
            $returnArr[$id] = array(
                'id' => $id,
                'method_name' => $methodArr->name,
                'active' => '1',
            );
        } else {
            $returnArr[$id] = array(
...
function update_get_methods(){
    global $Db, $_Proccess_Main_DB_Table;
        $fieldsArr = array(
            'method_name',
            'active',
            'last_update'
        );
        foreach ($_POST['methods'] as $method_name => $active){
            if($active == 'on'){
                $active = 1;
            } else {
                continue;
            }
            $str[] = "('" . $method_name . "', '" . $active . "', '" . time() . "')";
        }

        $query = "TRUNCATE TABLE `tb_get_methods`;";
        $result = $Db->query($query) or db_showError(__FILE__, __LINE__, $query);
        if(!empty($str)){
            $query	= "INSERT INTO `{$_Proccess_Main_DB_Table}` (`".implode('`, `', $fieldsArr)."`) VALUES ".implode(', ', $str);
            $result = $Db->query($query) or db_showError(__FILE__, __LINE__, $query);
        }
        module_updateStaticFiles();
}
...
function module_updateStaticFiles() {
    ...
    $UpdateStatic = new get_methodsUpdateStaticFiles();
...
<input type="button" class="buttons" onclick="javascript: location.href='?act=show<?php echo $fwParams;?>';" value="הצג הכל" />
<div class="maindiv">
	<br/>
	<? if($_Proccess_Has_GenricSearch){ ?>
		<br />
	...
	<?php  if($act=="show"){ ?>
		<div>
			...
			*
            המתודות שיסומן לידן צ׳קבוקס יישלחו למובייל ב-GD תחת המערך get_methodsArr

        </div>
        <form method="post" action="">
            <input type="hidden" name="update_get_methods" value="1">
            <button type="submit" id="update_get_methods">עדכן</button>
            <table width="100%" border="0" cellpadding="3" cellspacing="1" style="empty-cells:show;" class="table-list">
                ...
                foreach ($rowsArr as $row){
                    ...
                    <tr class="normTxt <?=$c?>" onmouseover="this.className='normTxt hover <?=$c?>';" onmouseout="this.className='normTxt <?=$c?>';">
                        ...
                        <td>
                            <input type="hidden" name="methods[<?=$row['method_name']?>]" value="<?=$row['active']?>">
                            <input type="checkbox" name="methods[<?=$row['method_name']?>]"<?=$row['active'] ? ' checked="checked"' : ''?>">
                        </td>
                        <?php  if($_Proccess_Has_Ordering_Action){ ?>
		...
		<?php  if ($Db->get_num_rows($result)==0){ ?>
			אין נתונים
		<?php  } ?>
	<?php  }elseif($act=="new"){ ?>
