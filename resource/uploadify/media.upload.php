...
$_REQUEST["Filename"] = $_FILES["Filedata"]["name"];
...
if (in_array(strtolower($img_ext), $image_types_arr)) {
    ...
    if ((!empty($_FILES))) {
        ...
        if ($w) {
            ...
            usleep(2);
            exit(json_encode(array('album_id' => $album_id, "item_id" => ($row['mobile'] && $resolution_id && $media_id && isset($line['title'])) ? $media_id : $item_media_id, 'ext' => $img_ext)));
        }
    }
}
